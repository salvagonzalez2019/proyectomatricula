...
try:
        ...
        print("Error Al Conector Con La Base De Datos", e)
...
def obtener():
    if (e_id.get() == ""):
        MessageBox.showwarning("Error","No Se Introdujo El ID Del Usuario Correctamente")
    else:
        con = mysql.connect(host='db4free.net',
                            database='alumnostecjilo',
                            user='rootmatricula',
...
def butone():
    ...
    nombrebase = Label(tabScanner, text="PLACA: ", font=('bold', 10)).place(x=50, y=100)
    matc = Label(tabScanner, text=matricula, font=('bold', 10)).place(x=150, y=100)
    ...
    custor = con.cursor()
    custor.execute(
        "select nombrealumno, apellidoalumno from alumno where matricula ='" + matricula + "'")
    rows = custor.fetchall()
    # cv2.imshow('image', img)
    # #cv2.imshow('Cropped', Cropped)
    for matricula in rows:
        nombrebase = Label(tabScanner, text="Nombre: ", font=('bold', 10)).place(x=50, y=130)
        nombreal = Label(tabScanner, text=matricula, font=('bold', 10)).place(x=150, y=130)
        checa = True
    ...
    if checa==True:
        global timer_id
        timer_id =  tablacontrol.after(3000, butone)
        checa = False

def buts():
    tablacontrol.after_cancel(timer_id)
    matc = Label(tabScanner, text="                    ", font=('bold', 10)).place(x=150, y=100)
    nombreal = Label(tabScanner, text="                                                       ",
                     font=('bold', 10)).place(x=150, y=130)
    MessageBox.showinfo("TESJI","Se Ha Detenido La Aplicacion Con Exito")

butonescan = Button(tabScanner,text='Empezar Scaneo',font=('italic', 10), bg="white", command =butone)
butonescan.place(x=40,y=50)
butonestop = Button(tabScanner,text='Termina Scaneo',font=('italic', 10), bg="white", command =buts)
butonestop.place(x=190,y=50)
root.mainloop()